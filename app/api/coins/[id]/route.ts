...
export async function PATCH(
  ...
  const { symbol, comment, plannedHoldingPeriod, tradeScore, buyEntries = [], sellEntries = [] } = body
  ...
  const trade = await prisma.$transaction(async (tx) => {
    ...
    return tx.coinTrade.update({
      where: { id },
      data: {
        symbol, comment: comment || null, plannedHoldingPeriod: plannedHoldingPeriod || null,
        tradeScore: tradeScore != null ? Number(tradeScore) : null,
        updatedAt: now,
