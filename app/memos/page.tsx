...
export default function MemosPage() {
  ...
  function startPinnedResize(e: React.MouseEvent) {
    ...
    function onMove(ev: MouseEvent) {
      ...
      setPinnedHeight(
        Math.max(120, Math.min(700, pinnedDragRef.current.startH + delta)),
      );
    }
    function onUp() {
  ...
  useEffect(() => {
    if (!formContent.trim() || stockMasters.length === 0) {
      setDetectedSymbols([]);
      return;
    }
  ...
  function openEdit(memo: Memo) {
    ...
    setFormRating(memo.rating);
    setFormCategory(memo.category);
    setFormAlertDate(memo.alertDate);
  ...
  async function handleEdit() {
    ...
    await apiFetch(`/api/memos/${selectedMemo.id}`, {
      ...
      body: JSON.stringify({
        ...
        category: formCategory,
        alertDate: formAlertDate,
        createdAt: formCreatedAt || undefined,
        symbols: formSymbols,
      }),
    });
    setModalMode("view");
  ...
  const alertCount = useMemo(
    () => memos.filter((m) => m.alertDate).length,
    [memos],
  );
  const stockPinCount = useMemo(
    () => memos.filter((m) => m.showOnMain).length,
    [memos],
  );
  const coinPinCount = useMemo(
    () => memos.filter((m) => m.showOnCoin).length,
    [memos],
  );
  ...
  const filtered = useMemo(() => {
    const list = memos.filter((m) => {
      ...
      if (
        ...
        !symbolFilters.some((f) => m.symbols.some((s) => s.symbol === f))
      )
        return false;
      ...
      if (dateFilter) {
        const field =
          dateFilterMode === "alertDate" ? m.alertDate : m.createdAt;
    ...
    <div className="min-h-screen">
      ...
      <header className="bg-white border-b px-4 py-3 flex justify-between items-center sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/"
            className="text-sm text-gray-500 hover:text-gray-800 px-3 py-1.5 rounded border"
          >
            ← 돌아가기
          </Link>
        </div>
      ...
      <div className="px-4 py-4 space-y-4">
          ...
          <div className="border border-orange-200 rounded-lg bg-orange-50 overflow-hidden">
            ...
            >
              <span className="text-xs font-medium text-orange-700">
                🔍 리뷰 필요{" "}
                <span className="ml-1 bg-orange-200 text-orange-800 px-1.5 py-0.5 rounded-full">
                  {reviewMemos.length}
                </span>
              </span>
            ...
            {reviewOpen && (
              <div className="border-t border-orange-200 divide-y divide-orange-100">
                {reviewMemos.map((memo) => {
                  const isOverdue =
                    memo.alertDate != null && memo.alertDate <= today;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {memo.category && (
                          <span
                            className={`text-xs px-1.5 py-0.5 rounded border ${CATEGORY_COLORS[memo.category] ?? "bg-gray-100 text-gray-500 border-gray-200"}`}
                          >
                            {memo.category}
                          </span>
                        )}
        ...
        {pinnedIds.length > 0 && (
          <div>
            ...
            >
              <div className="flex-none flex items-center justify-between px-3 py-2 bg-gray-50 border-b">
                <span className="text-xs font-medium text-gray-600">
                  열람 중{" "}
                  <span className="text-gray-400">{pinnedIds.length}개</span>
                </span>
                <button
              ...
              <div className="flex-1 flex gap-3 overflow-x-auto overflow-y-hidden p-3">
                {pinnedIds.map((id) => {
                  ...
                  return (
                    <div
                      key={id}
                      className="flex-none w-[36rem] border rounded-lg p-3 flex flex-col gap-2"
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button
                          ...
                          className="flex-none text-gray-300 hover:text-gray-500 text-base leading-none"
                        >
                          ✕
                        </button>
                      </div>
                      ...
                      <p className="text-xs text-gray-300 border-t pt-1.5 flex-none">
            ...
            >
              <div className="w-16 h-1 rounded-full bg-gray-100 group-hover:bg-gray-400 transition-colors" />
            </div>
          </div>
        )}
        ...
        {dateFilter && (
          <div className="flex items-center gap-2 px-1">
            <span
              className={`text-xs font-medium ${dateFilterMode === "alertDate" ? "text-orange-600" : "text-blue-600"}`}
            >
              {dateFilterMode === "alertDate" ? "🔔" : "📅"} {dateFilter}
            </span>
            <span className="text-xs text-gray-300">
              {dateFilterMode === "alertDate" ? "알림일" : "작성일"}
            </span>
            <button
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-1 flex-wrap">
            ...
            {stockPinCount > 0 && (
              <button
                onClick={() =>
                  setPinFilter(pinFilter === "stock" ? null : "stock")
                }
            ...
            {coinPinCount > 0 && (
              <button
                onClick={() =>
                  setPinFilter(pinFilter === "coin" ? null : "coin")
                }
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {memo.symbols.map((s) => (
                      <span
                        key={s.id}
                        className="text-xs bg-green-50 text-green-600 border border-green-200 px-1.5 py-0.5 rounded"
                      >
                        {s.symbol}
                      </span>
                    ))}
                  ...
                  {/* 내용 미리보기 */}
                  <p className="text-sm text-gray-700 flex-1 line-clamp-4 leading-relaxed">
                    {preview}
                  </p>
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-gray-300 mt-auto pt-1 border-t">
                    ...
                    {memo.showOnMain && (
                      <span className="bg-yellow-50 text-yellow-700 border border-yellow-200 px-1 py-0.5 rounded text-[10px]">
                        주식
                      </span>
                    )}
                    {memo.showOnCoin && (
                      <span className="bg-blue-50 text-blue-600 border border-blue-200 px-1 py-0.5 rounded text-[10px]">
                        코인
                      </span>
                    )}
      ...
      {modalMode !== "closed" && (
        ...
        >
          <div
            className="bg-white rounded-xl w-[92vw] max-w-3xl max-h-[88vh] flex flex-col shadow-xl"
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {selectedMemo.category && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${CATEGORY_COLORS[selectedMemo.category] ?? "bg-gray-100 text-gray-500 border-gray-200"}`}
                    >
                      {selectedMemo.category}
                    </span>
                  )}
                  {selectedMemo.rating != null && (
                  ...
                  {selectedMemo.alertDate && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${selectedMemo.alertDate <= today ? "bg-orange-50 text-orange-600 border-orange-200" : "bg-gray-50 text-gray-400 border-gray-200"}`}
                    >
                      🔔 {selectedMemo.alertDate}
                    </span>
                  )}
                ...
                <div className="flex gap-3 text-xs text-gray-300">
                  ...
                  {selectedMemo.updatedAt !== selectedMemo.createdAt && (
                    <span>수정 {selectedMemo.updatedAt.slice(0, 10)}</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleToggle(selectedMemo, "showOnMain")}
                  ...
                  <button
                    onClick={() => handleToggle(selectedMemo, "showOnCoin")}
                ...
                <div className="border-t pt-3">
                  <MemoImageZone
                    ...
                    images={getImages(selectedMemo)}
                    onUpdate={(imgs) => updateImages(selectedMemo.id, imgs)}
                  />
                </div>
              </div>
            )}
            ...
            {(modalMode === "add" || modalMode === "edit") && (
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  {formSymbols.length > 0 && (
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {formSymbols.map((s) => (
                        <span
                          key={s}
                          className="flex items-center gap-1 text-xs bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded-full"
                        >
                          {s}
                          <button
                            onClick={() =>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (stockMasters.some((m) => m.symbol === v)) {
                          if (!formSymbols.includes(v))
                            setFormSymbols((prev) => [...prev, v]);
                          setManualSymbolInput("");
                        }
                      }}
                      onKeyDown={(e) => {
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <button
                    onClick={closeModal}
                    className="text-sm text-gray-400 hover:text-gray-600 px-3 py-1.5 border rounded"
                  >
                    취소
                  </button>
                  <button
