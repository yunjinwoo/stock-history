...
function AlertDatePicker({ value, onChange }: { value: string | null; onChange: (v: string | null) => void }) {
  return (
    <div className="flex items-center gap-2">
      ...
      />
      {value && (
        <button type="button" onClick={() => onChange(null)} className="text-xs text-gray-300 hover:text-gray-500">
          초기화
        </button>
      )}
    </div>
...
export default function MemosPage() {
  ...
  const [newAlertDate, setNewAlertDate] = useState<string | null>(null)
  ...
  async function handleAdd() {
    ...
    await apiFetch('/api/memos', {
      ...
      body: JSON.stringify({ content: newContent, rating: newRating, category: newCategory, alertDate: newAlertDate }),
    })
    setNewContent('')
    setNewRating(null)
    setNewCategory(null)
    setNewAlertDate(null)
    setSaving(false)
    load()
  }
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2">
            {filtered.map(memo => {
              ...
              const isAlertPending = memo.alertDate != null && memo.alertDate > today
              return (
