...
export default function CoinFirstEntriesPage() {
  ...
  useEffect(() => {
    apiFetch('/api/coins')
      .then(r => r.json())
      .then((d: unknown) => Array.isArray(d) && setTrades(d as CoinTrade[]))
      .finally(() => setLoading(false))
  }, [])

  // 종목별로 전체 거래를 합산해 최초/최근 진입일 + 전체 체결 건수 계산
  const bySymbol = useMemo(() => {
    ...
    for (const t of trades) {
      ...
      for (const e of entries) {
        ...
        if (!cur) {
          map.set(t.symbol, { symbol: t.symbol, firstDate: date, lastDate: date, entryCount: 1 })
        } else {
          cur.entryCount++
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        <Link href="/coins" className="text-gray-400 hover:text-gray-700 text-sm">← 뒤로</Link>
        <h1 className="text-lg font-bold">코인 최초진입 타임라인</h1>
          ...
          {chartSymbol && (
            <div className="bg-white border rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-medium truncate">{chartSymbol}</p>
                <div className="flex items-center gap-2 shrink-0">
                  <a
                    href={`https://upbit.com/exchange?code=CRIX.UPBIT.KRW-${chartSymbol}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-blue-500 hover:text-blue-700 text-xs font-bold"
                    title="업비트 차트 열기"
                  >U</a>
                  <button onClick={() => setChartSymbol(null)} className="text-xs text-gray-400 hover:text-gray-600">닫기 ×</button>
                </div>
              </div>
              <TradingViewChart symbol={`UPBIT:${chartSymbol}KRW`} height={320} />
            </div>
          )}
        </div>
      </div>
