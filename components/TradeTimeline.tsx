...
// 수익률 구간별 색상 (진할수록 손익이 큼)
const RATE_TIERS = [
  { id: 'red-700',  label: '+20% 이상',   test: (r: number) => r >= 20,           border: 'border-l-red-700',  text: 'text-red-700',  dot: 'bg-red-700' },
  { id: 'red-500',  label: '+10 ~ 20%',  test: (r: number) => r >= 10,           border: 'border-l-red-500',  text: 'text-red-500',  dot: 'bg-red-500' },
  { id: 'red-300',  label: '0 ~ 10%',    test: (r: number) => r > 0,            border: 'border-l-red-300',  text: 'text-red-400',  dot: 'bg-red-300' },
  { id: 'gray-300', label: '0%',         test: (r: number) => r === 0,          border: 'border-l-gray-300', text: 'text-gray-500', dot: 'bg-gray-300' },
  { id: 'blue-300', label: '0 ~ -10%',   test: (r: number) => r > -10,          border: 'border-l-blue-300', text: 'text-blue-400', dot: 'bg-blue-300' },
  { id: 'blue-500', label: '-10 ~ -20%', test: (r: number) => r > -20,          border: 'border-l-blue-500', text: 'text-blue-500', dot: 'bg-blue-500' },
  { id: 'blue-700', label: '-20% 이하',   test: (_r: number) => true,            border: 'border-l-blue-700', text: 'text-blue-700', dot: 'bg-blue-700' },
] as const
...
export default function TradeTimeline({ trades, accounts, symbolTypeMap = {}, onEdit, onDelete }: Props) {
  ...
  const [weekOffset, setWeekOffset] = useState(0) // 0 = 이번 주가 가장 오른쪽
  ...
  const weeks = useMemo(() => {
    ...
    return Array.from({ length: WEEKS_SHOWN }, (_, i) => {
      const backFromNewest = weekOffset + (WEEKS_SHOWN - 1 - i)
    ...
    <div className="max-w-[1600px] mx-auto grid grid-cols-[160px_1fr] gap-4 items-start">
      ...
      <div className="space-y-3 min-w-0">
        <div className="flex items-center justify-between">
          <button onClick={() => setWeekOffset(o => o + 1)} className="px-3 py-1.5 text-gray-400 hover:text-gray-700 text-sm border rounded">‹ 이전주</button>
          <button
          ...
          {weeks.map((w, i) => (
            <div key={i} className="space-y-2 min-w-0">
              <div className="bg-gray-50 border rounded px-3 py-2 flex items-center justify-between">
                <span className="text-xs font-medium text-gray-600">{fmtMD(w.start)} ~ {fmtMD(w.end)}</span>
                {w.items.length > 0 && (
              ...
              {w.items.length === 0 ? (
                ...
                w.items.map(({ trade, exitDate, isWin }) => {
                  ...
                  return (
                    <div key={trade.id} className={`bg-white rounded-lg border-l-4 border overflow-hidden space-y-1.5 ${color.border}`}>
